    ...
    fetchChat: async (req, res) => {
        ...
        try {
            ...
            const Receiver = AllChats.map((chat) => {
                const receiver = chat.participants.find(participant => participant._id != LoggedInUserId);
                return {
                    receiver: {
                        fullName: receiver.fullName,
                        _id: receiver._id
                    },
                    latestMessage: chat.messages.length > 0 ? chat.messages[0].latestMessage : null,
                    lastCreated: chat.messages.length > 0 ? chat.messages[0].createdAt : null
                };
            });
