...
const UserController = {
    ...
    getAllUsers: async (req, res, next) => {
        try {
          const allUsers = await user.find().select('id fullName username');
      ...
      getProfile: async (req, res, next) => {
        try {
          ...
          if (!userProfile) {
            ...
            return res.status(200).json({ userProfile: { _id, fullName, username, email }, message: 'Profile fetched successfully' });
          }
